// Shared helpers for every GRID page.
(function () {
  ...
  const store = {
    get(k, d) { try { const v = localStorage.getItem('grid.' + k); return v == null ? d : JSON.parse(v); } catch { return d; } },
    set(k, v) { try { localStorage.setItem('grid.' + k, JSON.stringify(v)); } catch {} },
    del(k) { try { localStorage.removeItem('grid.' + k); } catch {} },
  };
  let marketP = null;
  const market = (fresh) => (!fresh && marketP) || (marketP = api('pairs'));
