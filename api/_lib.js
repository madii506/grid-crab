// GRID server library: http helpers, Solana RPC, Jupiter (tokens, prices, quotes, swaps), caches.
// Nothing here holds keys. Trades are built by Jupiter and signed in the user's own wallet.
const CFG = require('./_config');
const MOCK = () => globalThis.__GRID_MOCK || null;
const now = () => Date.now();
...
function wrap(fn) {
  return async (req, res) => {
    try { await fn(req, res); }
      ...
      console.error('[grid]', e && e.stack || e);
...
async function jup(path, opt = {}) {
  ...
  for (const h of JUP) {
    ...
    try { return await getJson(h + path, o, 12000); } catch (e) { last = e; if (e.status && e.status < 500 && e.status !== 401 && e.status !== 403 && e.status !== 404 && e.status !== 429) throw e; }
  }
  throw last || new Error('jupiter unreachable');
}

/* the tokens GRID trades against USDC, and where their price history comes from */
const PAIRS = [
  { mint: 'So11111111111111111111111111111111111111112', symbol: 'SOL', name: 'Solana', decimals: 9, cb: 'SOL-USD' },
  { mint: 'JUPyiwrYJFskUPiHa7hkeR8VUtAeFoSYbKedZNsDvCN', symbol: 'JUP', name: 'Jupiter', decimals: 6, cb: 'JUP-USD' },
  { mint: 'cbbtcf3aa214zXHbiAZQwf4122FBYbraNdFqgw4iMij', symbol: 'BTC', name: 'Coinbase Wrapped BTC', decimals: 8, cb: 'BTC-USD' },
  { mint: '7vfCXTUXx5WJV5JADk17DUJ4ksgau7utNKj4b963voxs', symbol: 'ETH', name: 'Ether (Portal)', decimals: 8, cb: 'ETH-USD' },
  { mint: 'DezXAZ8z7PnrnRJjz3wXBoRgixCa6xjnB7YaB1pPB263', symbol: 'BONK', name: 'Bonk', decimals: 5, cb: 'BONK-USD' },
  { mint: 'EKpQGSJtjMFqKZ9KQanSqYXRcF8fBopzLHYxdM65zcjm', symbol: 'WIF', name: 'dogwifhat', decimals: 6, cb: 'WIF-USD' },
  { mint: 'jtojtomepa8beP8AuQc6eXt5FriJwfFMwQx2v2f9mCL', symbol: 'JTO', name: 'Jito', decimals: 9, cb: 'JTO-USD' },
  { mint: 'HZ1JovNiVvGrGNiiYvEozEVgZ58xaU3RKwX8eACQBCt3', symbol: 'PYTH', name: 'Pyth Network', decimals: 6, cb: 'PYTH-USD' },
  { mint: '4k3Dyjzvzp8eMZWUXbBCjEvwSkkk59S5iCNLY3QrkX6R', symbol: 'RAY', name: 'Raydium', decimals: 6, cb: null },
];
...
module.exports = { PAIRS, CFG, send, body, query, ip, Fail, wrap, limit, isAddr, getJson, rpc, jup, SOL, USDC, KNOWN, classify, tokenMeta, prices, stockList, now };
